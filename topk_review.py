...
def display_searched_papers_component():
    #すでに papers のデータフレームがあれば、それを表示する。
    if 'papers_df' in st.session_state:
        if 'number_of_review_papers' in st.session_state:
            display_dataframe_detail(st.session_state['papers_df'],  f"論文検索結果上位 {st.session_state['number_of_review_papers']}件", st.session_state['number_of_review_papers'])
        else:
            display_dataframe_detail(st.session_state['papers_df'], f'論文検索結果上位 20 件', 20)

        # クラスターのレビュー生成
        number_of_papers = st.slider(
            f"レビューに使用する論文数を選択してください。", min_value=1, value=20, max_value=min(100, len(st.session_state['papers_df'])),
            step=1)
    ...
    if 'papers_df' in st.session_state:
        ...
        if topk_review_button:
            with st.spinner("⏳ AIによるレビューの生成中です。 お待ち下さい..."):
                # response, links, caption, draft_references = title_review_papers(st.session_state['papers_df'][:st.session_state['number_of_review_papers']], st.session_state['query'], model = 'gpt-4-1106-preview', language=toggle)
                response, links, caption, draft_references = streamlit_title_review_papers(st.session_state['papers_df'][:st.session_state['number_of_review_papers']], st.session_state['query'], model = 'gpt-4-1106-preview', language=toggle)
...
def display_topk_review_component():
    ...
    if 'papers_df' in st.session_state and 'topk_review_response' in st.session_state and 'topk_review_caption' in st.session_state and 'topk_references_list' in st.session_state:
        ...
        display_spaces(1)
        st.markdown(st.session_state['topk_review_response'], unsafe_allow_html=True)
    ...
    if 'papers_df' in st.session_state and 'topk_review_response'in st.session_state and 'topk_review_caption' in st.session_state and 'topk_references_list' in st.session_state:
        ...
        if next_topk_review_button:
            ...
            with st.spinner(f"⏳ {button_title} の論文を使用した AI によるレビューの生成中です。 お待ち下さい..."):
                response, links, caption, draft_references = streamlit_title_review_papers(
                    ...
                    st.session_state['query'], model='gpt-4-1106-preview', language=st.session_state['topk_review_toggle'] )
...
def generate_topk_draft_component():
    if 'papers_df' in st.session_state:
        ...
        if write_summary_button and len(draft_text) > 0:
            if 'topk_review_response' in st.session_state and len(st.session_state['topk_review_response']) > 0 and 'topk_draft_references_list' in st.session_state:
                with st.spinner("⏳ AIによるエビデンスの付与中です。 お待ち下さい..."):
                    topk_summary_response, caption = streamlit_summery_writer_with_draft(st.session_state['topk_review_response'], draft_text, st.session_state['topk_draft_references_list'], model = 'gpt-4-1106-preview', language=toggle, mode=mode_toggle)
...
def display_topk_draft_component():
    if 'topk_summary_response' in st.session_state and 'topk_summary_references_list' in st.session_state:
        st.markdown(st.session_state['topk_summary_response'], unsafe_allow_html=True)
...
def topk_review_papers():
    display_searched_papers_component()
